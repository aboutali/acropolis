...
window.buildMats = function (THREE) {
  ...
  // realism phase 2: these procedural fields are now only the instant-first-frame fallback
  // (assets/tex/*.jpg swap in moments later via mats.finishScene's progressive loader), so
  // the synthesis budget is cut well below the old "final" quality target.
  var SIZE = 512;                      // final texture size
  var GEN = MOBILE ? 160 : 256;        // synthesis resolution for the primary (hero) fields
  var GEN_S = MOBILE ? 112 : 176;      // synthesis resolution for secondary fields
  var ALL_TEX = [];
  ...
  var UV_SCALE = {
    ...
    rock: 8, rockDark: 9, ground: 5, city: 4, terracotta: 1.4,
  ...
  // ---------------------------------------------------------------
  // realism phase 2: progressive photo-texture loader. The procedural CanvasTextures above
  // render instantly on first frame; this quietly starts real CC0 photo-scanned PBR textures
  // (assets/tex/*.jpg — see assets/CREDITS.md) loading in the background and swaps each map
  // in the moment it arrives. A texture that 404s or errors simply never swaps in, so the
  // procedural fallback silently stays -- no error is surfaced to the scene.
  // ---------------------------------------------------------------
  function loadRealTextures(maxAniso) {
    if (!HAS_DOC || typeof THREE.ImageLoader !== 'function') return;
    var loader = new THREE.ImageLoader();
    // Every mats.X.map/normalMap/roughnessMap below is already a real THREE.Texture object
    // (the procedural CanvasTexture built above). Several modules (03/08/09) call
    // mats.X.clone() at scene-build time, *before* this async loader resolves; clone() only
    // copies the texture *reference*, so mutating that shared Texture's own .image in place
    // (never reassigning mats.X.map to a brand-new Texture object) is what makes every
    // existing clone pick up the swap too, with no extra bookkeeping.
    function loadInto(tex, url, tint) {
      if (!tex) return;
      loader.load(url, function (img) {
        ...
        if (tint) {
          ...
          for (var i = 0; i < d.length; i += 4) {
            d[i] = Math.min(255, d[i] * tint[0]); d[i + 1] = Math.min(255, d[i + 1] * tint[1]); d[i + 2] = Math.min(255, d[i + 2] * tint[2]);
          }
      ...
      }, undefined, function () { /* keep the procedural image: no-op */ });
    }
    // Loads one real map onto every material's existing texture object in matList.
    function apply(matList, prop, url, tint) {
      for (var i = 0; i < matList.length; i++) loadInto(matList[i][prop], url, tint);
    }
    // Loads a full {col,nor,rough} set (assets/tex/<base>_{col,nor,rough}[suffix].jpg) onto matList.
    function set(matList, base, colSuffix) {
      apply(matList, 'map', 'assets/tex/' + base + '_col' + (colSuffix || '') + '.jpg');
      apply(matList, 'normalMap', 'assets/tex/' + base + '_nor.jpg');
      apply(matList, 'roughnessMap', 'assets/tex/' + base + '_rough.jpg');
    }
    // Marble: 2k on desktop for the hero material only (columns/walls/entablature); mobile
    // and every other marble-family variant share the 1k set. Worn/shadowed/relief variants
    // keep their own weathered/shadow colour by baking a multiply-tint into the swapped
    // image itself (not material.color, which -- like the map reference -- is already
    ...
    // marbleStatue keeps its own (smoother) procedural roughnessMap rather than the block
    // photo's roughness, per the "smoother variant for statues" spec.
    set([mats.rock], 'rock');
  ...
  mats.finishScene = function (scene, renderer) {
    ...
    if (!realTexturesStarted) { realTexturesStarted = true; try { loadRealTextures(maxAniso); } catch (e) {} }
